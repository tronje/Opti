        ...
        \item[\textbf{1.}]
            \begin{enumerate}
                \item[a)]
                    ...
                    % Wir setzen diese vermeintlich optimale Lösung in das Hauptproblem von (D) ein, und erhalten als Ergebnis $18$. Setzt man die vorgeschlagene
                    % Lösung in (P) ein, so erhält man ebenfalls $18$ als Ergebnis. Nun liegt es nahe zu folgern, dass die vorgeschlagene Lösung tatsächlich
                    % optimal ist. Aber: wir haben vergessen, die erhaltene Lösung von (D) in die Nebenbedingungen einzusetzen! Tut man dies, so erhält man für die
                    % erste Nebenbe
                \item[b)]
